###########################################################
## plot.gM.perGen.zoom.R: Script similar to plot.perGen.R 
## but to be used with the "File/ Custom plot" option 
## allows choosing the first and last reproductive events to visualize 
## last modified 24/10/21 ## Pauline GARNIER-GERE
## script available in the *.exe folder
...
## NB: Updated scripts after the first release will be uploaded on the gMetapop website
...
# Computing mean for h2.pan h2.c and fitness if statistics are requested in per_pop output file
  ...
  if (length(list.files(, all.files=F,full.names=F, recursive=F, pattern="res1_per_pop_1.txt"))!=0) { #if res1 per pop1 present in workdir
     ...
     warn.count.00<-0
     for (sst in pr.stats.pop) { #  sst<-"Fis"
